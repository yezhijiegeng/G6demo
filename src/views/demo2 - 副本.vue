...
export default {
  ...
  methods: {
    ...
    customNode() {
      const _this = this
      register(ExtensionCategory.NODE, 'icon-node', {
        draw(cfg, group) {
    ...
    customEdge() {
      const _this = this;
      register(ExtensionCategory.EDGE, 'flow-line', {
        draw(cfg, group) {
  ...
  mounted() {
    ...
    // 解决渲染残影问题
    // this.graph.getOptions('canvas').setOptions('localRefresh', false)
    this.graph.setData(ghData)
    this.graph.render();
