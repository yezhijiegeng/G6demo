      ...
      G6.registerNode("icon-node", {
        ...
        setState(name, value, item) {
          if (name === "collapsed3") {
            ...
            const collapseNode = group.find(
              (ele) => ele.get("name") === "collapse-node3"
            );
            collapseNode.attr({
            ...
            const group = item.getContainer();
            const collapseNode = group.find(
              (ele) => ele.get("name") === "collapse-node1"
            );
            collapseNode.attr({
              symbol: value ? EXPAND_ICON : COLLAPSE_ICON,
            });
            ...
            const group = item.getContainer();
            const collapseNode = group.find(
              (ele) => ele.get("name") === "collapse-node2"
            );
            collapseNode.attr({
              symbol: value ? EXPAND_ICON : COLLAPSE_ICON,
            });
            ...
            const group = item.getContainer();
            const collapseNode = group.find(
              (ele) => ele.get("name") === "collapse-node-top"
            );
            collapseNode.attr({
              symbol: value ? EXPAND_ICON : COLLAPSE_ICON,
            });
  ...
  mounted() {
    ...
    const handleCollapse = (e, name) => {
      ...
      const getFindResNodesIds = () => {
        let resArr = [];
        // 寻找该节点下一层公司节点的所有子节点
        let targetCurNodes = ghData.edges.filter(
          (el) => el.source === curNodeId
        );
        targetCurNodes = targetCurNodes.map((el) => el.target);
        let curNodes = [];
        if (name === "collapse-node1") {
          curNodes = ghData.nodes.filter(
            (el) => targetCurNodes.includes(el.id) && !el.staff
          );
